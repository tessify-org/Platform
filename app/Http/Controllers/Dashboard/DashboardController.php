...
class DashboardController extends Controller
{
    public function getDashboard()
    {
        ...
        if ($now->hour > 0 and $now->hour < 6) {
            $greeting = __("dashboard.greeting_morning", ["name" => $user->formattedName]);
        } elseif ($now->hour > 6 and $now->hour < 12) {
            $greeting = __("dashboard.greeting_afternoon", ["name" => $user->formattedName]);
        } elseif ($now->hour > 12 and $now->hour < 18) {
            $greeting = __("dashboard.greeting_evening", ["name" => $user->formattedName]);
        } else {
            $greeting = __("dashboard.greeting_night", ["name" => $user->formattedName]);
        }
        
        // Subscriptions & followings
        $followings = $user->followings;
        $subscribedMinistries = $user->subscriptions()->withType("App\\Models\\Ministry")->get();
        $subscribedOrganizations = $user->subscriptions()->withType("App\\Models\\Organization")->get();
        $subscribedProjects = $user->subscriptions()->withType("App\\Models\\Project")->get();
        $subscribedTasks = $user->subscriptions()->withType("App\\Models\\Task")->get();
